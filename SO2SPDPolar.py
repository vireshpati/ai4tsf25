...
class Model(nn.Module):
    """
    SO(2)-SPD-SO(2) Polar Decomposition 
...
class SO2SPDPolarLayer(nn.Module):
    ...
    def softmax_attention_scan(self, q, k, v):
        out = F.scaled_dot_product_attention(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2), is_causal=True, dropout_p=self.dropout).transpose(1, 2)
        return out
...
def apply_rope(x: torch.Tensor, times: torch.Tensor, num_heads: int = 1) -> torch.Tensor:
    """
    Apply Rotary Position Embedding (RoPE) to input tensor with multi-head support

    Args:
        x: Input tensor [B, T, D] or [B, H, T, D_head]
        times: Time positions [T]
        num_heads: Number of attention heads (default: 1 for single-head)
    ...
    # Handle multi-head case: [B, H, T, D_head] -> [B*H, T, D_head]
    if len(input_shape) == 4:
        B, H, T, D_head = input_shape
        x = x.reshape(B * H, T, D_head)
        D = D_head
    else:
    ...
    x_reshaped = x[..., :2*half_dim].view(-1, T, half_dim, 2)
    x1 = x_reshaped[..., 0]  # [B(*H), T, D//2]
    x2 = x_reshaped[..., 1]  # [B(*H), T, D//2]
    ...
    x_rot = torch.stack([x1_rot, x2_rot], dim=-1).view(-1, T, 2*half_dim)
